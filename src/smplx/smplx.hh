...
namespace smplx
{
     ...
     *  model and handles loading from a standard SMPL-X cbor file.
     *
    ...
    public:
        ...
        explicit Model(
            ...
            Gender model_gender,
            const std::filesystem::path& model_uv_path = ""/* optional */
        );
        ...
        inline size_t n_uv_verts() const noexcept { return _n_uv_verts; }
        ...
        inline bool has_uv_map() const noexcept { return _n_uv_verts > 0; }
    ...
    public:
        ...
        // Save as obj file
        void save_obj(const std::filesystem::path& path) const;
        ...
        inline auto trans() noexcept { return params.template head<3>(); }
        inline auto trans() const noexcept { return params.template head<3>(); }

        // Pose (angle-axis)
        inline auto pose() noexcept { return params.template segment<ModelConfig::n_explicit_joints() * 3>(3); }
        inline auto pose() const noexcept { return params.template segment<ModelConfig::n_explicit_joints() * 3>(3); }

        // Hand principal component weights
        inline auto hand_pca() noexcept {
            return params.template segment<ModelConfig::n_hand_pca() * 2>(
                3 + 3 * model.n_explicit_joints()
            );
        }
        inline auto hand_pca() const noexcept {
            return params.template segment<ModelConfig::n_hand_pca() * 2>(
                3 + 3 * model.n_explicit_joints());
        }
        inline auto hand_pca_l() noexcept {
            return params.template segment<ModelConfig::n_hand_pca()>(
                3 + 3 * model.n_explicit_joints());
        }
        inline auto hand_pca_l() const noexcept {
            return params.template segment<ModelConfig::n_hand_pca()>(
                3 + 3 * model.n_explicit_joints());
        }
        inline auto hand_pca_r() noexcept {
            return params.template segment<ModelConfig::n_hand_pca()>(
                3 + 3 * model.n_explicit_joints() + model.n_hand_pca());
        }
        inline auto hand_pca_r() const noexcept {
            return params.template segment<ModelConfig::n_hand_pca()>(
                3 + 3 * model.n_explicit_joints() + model.n_hand_pca());
        }

        // Shape params
        inline auto shape() noexcept { return params.template tail<ModelConfig::n_shape_blends()>(); }
        inline auto shape() const noexcept { return params.template tail<ModelConfig::n_shape_blends()>(); }
        ...
        inline void set_zero() noexcept { params.setZero(); }

        // Set parameters uar in [-0.25, 0.25]
        inline void set_random() noexcept { params.setRandom(); params *= 0.25f; }
