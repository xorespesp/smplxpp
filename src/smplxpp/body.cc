...
namespace smplxpp
{
    ...
    // Main LBS routine
    template <class ModelConfig>
    void Body<ModelConfig>::update([[maybe_unused]] bool force_cpu, bool enable_pose_blendshapes) {
        // _SMPLXPP_BEGIN_PROFILE;
        ...
        if constexpr (ModelConfig::n_hand_pca_joints() > 0) {
        ...
        _local_to_global();
        // _SMPLXPP_PROFILE(localglobal);

        // * LBS *
        // Construct a transform for each vertex
        _vert_transforms.noalias() = model.weights * _joint_transforms;
        // _SMPLXPP_PROFILE(lbs weight computation);
        ...
        // _SMPLXPP_PROFILE(lbs point transform);
    }
    ...
    void Body<ModelConfig>::_local_to_global() {
        ...
        for (size_t i = 0; i < ModelConfig::n_joints(); ++i) {
    ...
    template <class ModelConfig>
    void Body<ModelConfig>::save_obj(const std::filesystem::path& path) const {
        const auto& cur_verts = verts();
        if (cur_verts.rows() == 0) return;
        std::ofstream ofs(path);
        ofs << "# Generated by SMPLXPP"
            << "\n";
        ofs << std::fixed << std::setprecision(6) << "o smplxpp\n";
        for (size_t i = 0; i < model.n_verts(); ++i) {
